    ...
    <div className="flex h-screen bg-gray-50 font-sans text-gray-900 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <footer className="bg-white border-t border-gray-100 px-6 py-2.5 flex items-center justify-center gap-2">
          <RolePill label="Admin" active={activeRole === "admin"} onClick={() => setActiveRole("admin")}
            icon={<svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></svg>}
          />
          <RolePill label="Réception" active={activeRole === "reception"} onClick={() => setActiveRole("reception")}
            icon={<svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21" /></svg>}
          />
          <RolePill label="Client" active={activeRole === "client"} onClick={() => setActiveRole("client")}
